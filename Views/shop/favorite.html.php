							...
							<div class="cart_items">
								<ul class="cart_list">
									<?php if(isset($_SESSION['fav'])){ ?>
										<?php foreach ($_SESSION['fav'] as $fav) { ?>
											<li class="cart_item clearfix" id="li-<?php echo $fav['combo_id'] ?>">
												<div class="cart_item_image" style="overflow: hidden;"><img src="<?php echo $fav['combo_image_name'] ?>" alt=""></div>
												<div class="cart_item_info d-flex flex-md-row flex-column justify-content-between">
													<div class="cart_item_name cart_info_col">
														<div class="cart_item_title">Nombre</div>
														<div class="cart_item_text"><?php echo ucwords($fav['combo_name']) ?></div>
													</div>
													<div class="cart_item_quantity cart_info_col">
														<div class="cart_item_title">Categoria</div>
														<div class="cart_item_text"><?php echo ucwords($fav['combo_type_name']) ?></div>
													</div>
													<div class="cart_item_price cart_info_col">
														<div class="cart_item_title">Precio</div>
														<div class="cart_item_text">$ <?php echo $fav['combo_sale_price'] ?></div>
														<input type="hidden" id="price-<?php echo $fav['combo_id'] ?>" value="<?php echo $fav['combo_sale_price']?>">
													</div>
													<div class="cart_item_total cart_info_col">
														<div class="cart_item_title">Acciones</div>
														<input id="quantity_input" type="hidden" value="1">
														<div class="cart_item_text"><a data-url="?c=cart&a=add&combo_id=<?php echo $fav['combo_id']; ?>" class="btn btn-primary add-cart"><i class="fa fa-shopping-cart"></i></a> <a href="?c=shop&a=product&combo_id=<?php echo $fav['combo_id'] ?>" title="Ver combo" class="btn btn-info"><i class="fa fa-eye"></i></a> <a href="" title="Quitar combo" class="btn btn-danger btn-delete" data-id="<?php echo $fav['combo_id'] ?>"><i class="fa fa-trash-alt"></i></a></div>
													</div>
												</div>
											</li>
											<?php } ?>
							...
							<div class="row">
								<?php if (isset($_SESSION['fav'])) { ?>
									<div class="cart_buttons width-50p col-lg-12">
										<a href="?c=shop&a=index" class="button cart_button_clear" id="add-favorites"><i class="fa fa-plus-circle"></i> Agregar <span class="hidden-max-992px">más</span></a>
									</div>
								<?php }else{?>
									<div class="cart_buttons width-50p col-lg-12">
										<a href="?c=shop&a=index" class="button cart_button_clear" id="add-favorites"><i class="fa fa-plus-circle"></i> Agrega <span class="hidden-max-992px">tu primer combo</span></a>
									</div>
								<?php } ?>
							</div>						
						</div>
...
<script src="public/js/shop/favorite.js"></script>
